...
- 강의의 실습 문제는 원본이 영어로 제공되나, **모든 문제를 한글화하여 자체적으로 재구성**하여 풀이하였습니다.
- 파일을 다운로드받아 수행하는 방식의 실습은 원본 그대로 활용하되, 그 외의 실습 문제는 **감사(Audit) 업무와 연관된 질문과 시나리오**로 대체하여 실전 맥락에서 학습하였습니다.
- 예시: 차입금 약정 위반 시 유동/비유동 분류 판단, 미지급금 감사절차 수행, 감사조서 작성 등
